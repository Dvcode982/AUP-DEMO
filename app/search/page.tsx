...
export default function SearchPage() {
  ...
  const currentSearchTerm = searchQuery || searchTag
    ...
    <div className="flex h-screen bg-gradient-to-br">
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <div className="max-w-6xl mx-auto">
          <h1 className="text-3xl font-bold mb-6 text-gray-800 dark:text-gray-200">{t('searchResults')}</h1>
          ...
          <div className="mb-8">
            <h2 className="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-200">
              搜索结果 {!loading && posts.length > 0 && `(${posts.length})`}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {loading ? (
                <div className="col-span-full text-center py-10">
                  <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500 mx-auto"></div>
                  <p className="mt-4 text-gray-500 dark:text-gray-400">{t('searching')}</p>
                </div>
              ) : posts.length > 0 ? (
                posts.map(post => (
