...
import { Home, Search, User, Palette, HelpCircle, LogIn, LogOut, Sun, Moon, Contact, Shapes, Settings } from 'lucide-react'
...
const Sidebar = () => {
  ...
  const menuItems = [
    ...
    { href: '/friends', label: t('nav.friends'), icon: User },
    ...
    <aside className="flex flex-col w-16 md:w-64 h-screen bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 transition-all duration-300">
      ...
      <div className="p-4 bg-white dark:bg-gray-800">
        {isAuthenticated && user ? (
          <Link href="/my-profile" className="flex items-center space-x-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-all duration-200 shadow-sm border border-gray-100 dark:border-gray-600">
            <Avatar className="h-10 w-10 md:h-12 md:w-12">
              <AvatarImage 
                src={user.avatar || '/images/lon.jpg'} 
            ...
            <div className="flex-1 hidden md:block">
              <div className="font-semibold text-gray-800 dark:text-gray-100 flex items-center gap-1">
          ...
          <Link href="/login" className="flex items-center space-x-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-all duration-200 shadow-sm border border-gray-100 dark:border-gray-600">
            <Avatar className="h-10 w-10 md:h-12 md:w-12">
              <AvatarImage src="/images/lon.jpg" alt="Default avatar" />
        ...
        <div className="bg-gradient-to-r from-transparent via-gray-200 dark:via-gray-600 to-transparent h-[1px] hidden md:block mt-4"></div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-4">
        ...
        <div className="px-2 mt-4 space-y-2">
          ...
          >
            {theme === 'dark' ? <Sun className="w-5 h-5 md:mr-3" /> : <Moon className="w-5 h-5 md:mr-3" />}
            <span className="hidden md:inline font-medium">{theme === 'dark' ? t('settings.theme.light') : t('settings.theme.dark')}</span>
          </button>
          ...
          >
            ...
            <span className="hidden md:inline font-medium">{isCustomBackground ? t('settings.background.off') : t('settings.background.on')}</span>
          </button>
        </div>
