...
export default function TopicRecommendations() {
  ...
  if (!isAuthenticated) {
    return (
      <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-lg p-3 shadow-sm border border-gray-100 dark:border-gray-700">
        <h3 className="text-gray-500 dark:text-gray-100 text-sm font-semibold mb-2 flex items-center gap-1.5">
  ...
  const getTopicColor = (topic: string) => {
    ...
    return colors[topic] || 'text-blue-600 bg-blue-50 dark:text-blue-400 dark:bg-blue-900/30'
  }

  // 取第一个推荐主题用于变色
  const mainTopic = recommendations.length > 0 ? recommendations[0].topic : '';

  return (
    <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-lg p-3 shadow-sm border border-gray-100 dark:border-gray-700">
      <h3 className={`text-sm font-semibold mb-3 flex items-center gap-1.5 ${getTopicColor(mainTopic)}`}>
        <Sparkles className="w-4 h-4" />
        为您推荐
      </h3>
