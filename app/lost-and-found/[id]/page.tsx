...
export default function LostAndFoundItemPage() {
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 调用API添加评论
      await lostAndFoundAPI.addLostAndFoundComment(id, newComment)
      
      // 重新获取评论列表
      const commentsData = await lostAndFoundAPI.getLostAndFoundComments(id)
  ...
  const handleMarkAsReturned = async () => {
    ...
    try {
      ...
      // 调用API标记为已归还
      await lostAndFoundAPI.markAsReturned(id)
      
      // 重新获取物品详情
      const itemData = await lostAndFoundAPI.getLostAndFoundItemById(id)
      setItem(prev => prev ? { ...prev, ...itemData, isReturned: true } : null)
      
      // 添加系统评论
      await lostAndFoundAPI.addLostAndFoundComment(id, '物品已标记为找到/归还')
  ...
  const handleCancelReturned = async () => {
    ...
    try {
      ...
      // 调用API取消已归还状态
      await lostAndFoundAPI.cancelReturned(id)
      ...
      // 添加系统评论
      await lostAndFoundAPI.addLostAndFoundComment(id, '已解决状态已取消，物品重新标记为待解决')
      ...
      <main className="flex-1 pt-4 pb-4 pl-4 pr-4 overflow-hidden">
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center space-x-4 mb-4">
            <button
              onClick={() => router.back()}
              className="flex items-center justify-center w-10 h-10 rounded-xl bg-white/80 dark:bg-gray-800/80 hover:bg-white dark:hover:bg-gray-800 transition-colors border border-gray-200/50 dark:border-gray-700/50"
            >
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center space-x-2 mt-1">
                ...
                {item.category && (
                  <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-white/60 dark:bg-gray-700/60 text-gray-700 dark:text-gray-300">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                onClick={handleShare}
                className="flex items-center space-x-1 px-3 py-2 rounded-xl text-sm font-medium bg-white/80 dark:bg-gray-800/80 text-gray-600 dark:text-gray-300 hover:bg-white dark:hover:bg-gray-800 transition-colors"
              >
          ...
          {/* 主要内容 */}
          <div className="flex-1 min-h-0 overflow-y-auto bg-white/40 dark:bg-gray-800/40 rounded-lg p-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              {/* 左侧：主要信息 */}
              <div className="lg:col-span-2 space-y-6">
                {/* 物品信息卡片 */}
                <div className="bg-white/80 dark:bg-gray-800/80 rounded-xl border border-gray-200/50 dark:border-gray-700/50 p-6">
                  {/* 发布者信息 */}
